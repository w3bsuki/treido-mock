  ...
  return (
    <div className="bg-white min-h-screen pb-24 font-sans relative">
      ...
      <div className="sticky top-0 z-40 bg-white/95 backdrop-blur-xl border-b border-zinc-200">
        ...
        <div className="pt-safe-top">
          <div className="flex items-center justify-between px-3 h-[48px]">
            <div className="flex items-center">
              ...
              <h1 className="text-[16px] font-bold text-zinc-900 ml-1 leading-none transform translate-y-[0.5px]">
                {categoryName}
              </h1>
            </div>
        ...
        {/* L1 NAVIGATION: Gender Tabs - Compact Gap */}
        {isFashion && (
            <div className="flex items-center px-3 gap-5 border-b border-zinc-100 overflow-x-auto no-scrollbar">
                {FASHION_GROUPS.map((group) => {
                    ...
                    return (
                        <button
                            ...
                            className={`
                                relative py-2.5 text-[13px] whitespace-nowrap transition-colors flex-shrink-0
                                ${isActive 
        ...
        <div className="bg-white">
           ...
           {/* STATE 1: Circles (Default) - Tight Padding */}
           {!activeDept && isFashion && (
             <div className="py-3 overflow-x-auto no-scrollbar px-3 animate-in fade-in duration-300">
           ...
           {(activeDept || !isFashion) && (
             <div className="py-2.5 flex items-center overflow-x-auto no-scrollbar px-3 gap-2 animate-in fade-in slide-in-from-right-2 duration-300">
                 ...
                 {isFashion && activeDeptObj && ActiveIcon && (
                   <>
                      <button 
                        ...
                        className="flex-shrink-0 flex items-center gap-1.5 pl-2 pr-3 py-1.5 bg-zinc-900 text-white rounded-full text-[12px] font-medium active:opacity-90 transition-opacity shadow-sm"
                      >
                        <div className="w-4 h-4 bg-white/20 rounded-full flex items-center justify-center">
                            <ActiveIcon className="w-2.5 h-2.5 stroke-[2]" />
                        </div>
                      ...
                      {/* Divider */}
                      <div className="w-[1px] h-4 bg-zinc-200 mx-1 flex-shrink-0"></div>
                   </>
                 )}
                 ...
                 {isFashion ? (
                   <>
                    ...
                    SUB_CATEGORIES.map(sub => (
                       <button key={sub.id} className="whitespace-nowrap px-3 py-1.5 rounded-full text-[12px] font-medium transition-all border bg-white text-zinc-600 border-zinc-200">
                          {sub.name}
                       </button>
                    ))
                 )}
      ...
      <div className="px-3 pt-3">
         {/* Breadcrumb Context */}
         <div className="mb-2.5 px-1 flex items-center gap-1.5 text-[10px] font-medium text-zinc-400">
            <span>Мода</span>
