...
export const BottomNav: React.FC = () => {
  return (
    // Super Compact: h-[44px] content box + safe area
    <nav className="fixed bottom-0 z-50 w-full max-w-[430px] mx-auto bg-white/95 backdrop-blur-xl border-t border-gray-100 pb-safe">
      <div className="grid grid-cols-5 items-center px-1 h-[50px]">
        ...
        {/* Sell Tab */}
        <button className="flex flex-col items-center justify-center gap-0.5 group active:scale-95 transition-transform p-1 w-full text-gray-900">
...
const NavItem: React.FC<{ icon: React.ReactNode; label: string; isActive?: boolean }> = ({ icon, label, isActive }) => {
  return (
    <button className={`flex flex-col items-center justify-center gap-0.5 group active:scale-95 transition-transform p-1 w-full ${isActive ? 'text-gray-900' : 'text-gray-400 hover:text-gray-900'}`}>
