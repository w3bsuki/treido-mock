  ...
  return (
    <div className="min-h-screen bg-white pb-[80px]">
      ...
      <div className="sticky top-0 z-50 bg-white border-b border-zinc-200">
        <div className="pt-safe-top px-3 pb-3">
          <div className="h-[52px] flex items-center gap-3">
            <div className="relative flex-1 group">
              ...
              <input
                ...
                className="block w-full h-[40px] pl-9 pr-10 bg-zinc-100 border border-transparent focus:bg-white focus:border-zinc-900 focus:ring-0 rounded-md text-[15px] text-zinc-900 placeholder-zinc-400 transition-all font-medium outline-none"
        ...
        {query.length === 0 ? (
          <div className="animate-in fade-in slide-in-from-bottom-2 duration-500">
            {recentSearches.length > 0 && (
              <div className="mb-6">
                <div className="px-4 mb-2 flex items-center justify-between">
                  <h3 className="text-[12px] font-bold text-zinc-900 uppercase tracking-wide">Скорошни</h3>
                  <button onClick={() => setRecentSearches([])} className="text-[12px] font-medium text-zinc-400 hover:text-zinc-900">Изчисти</button>
                </div>
                <div>
                  {recentSearches.map((term) => (
                    <div 
                      ...
                      className="flex items-center justify-between px-4 py-3 active:bg-zinc-50 transition-colors cursor-pointer border-b border-zinc-50 last:border-0"
                    >
            ...
            <div className="mb-8 px-4">
               <h3 className="text-[12px] font-bold text-zinc-900 uppercase tracking-wide mb-3 flex items-center gap-2">
            ...
            <div className="px-4">
               <h3 className="text-[12px] font-bold text-zinc-900 uppercase tracking-wide mb-3">Разгледай</h3>
               <div className="grid grid-cols-2 gap-2">
                 {CATEGORIES.slice(1, 5).map(cat => (
                   <button key={cat.id} className="h-[70px] bg-zinc-50 rounded-md border border-zinc-100 p-3 flex flex-col justify-between items-start active:scale-[0.98] transition-transform">
                      <span className="font-bold text-zinc-900 text-[14px]">{cat.name}</span>
